...
NEW_TASK_DEFINTIION=$(echo $test | jq --arg IMAGE "$NEW_IMAGE" '.taskDefinition | .containerDefinitions[0].image = "692851696394.dkr.ecr.us-east-1.amazonaws.com/test:$BUILD_NUMBER" | del(.taskDefinitionArn) | del(.revision) | del(.status) | del(.requiresAttributes) | del(.compatibilities)')
